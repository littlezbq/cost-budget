...
class ZDModelLoader:
    ...
    def load_all(self):
        if not self.model_root_dir.exists():
            raise FileNotFoundError(f"❌ ZD模型目录不存在 → {self.model_root_dir}")
...
# ===================== ZD专属预测类 =====================
class ZDPredictor:
    def __init__(self, loader: ZDModelLoader, X):
        if loader.model_material is None:
            raise ValueError("❌ ZD模型未加载，请先调用 loader.load_all()")
        self.loader = loader
        self.X = X
...
# ===================== ZD专属SHAP分析函数 =====================
def generate_zd_shap_analysis(data_dict: dict, model_loader: ZDModelLoader, save_dir="."):
    """SHAP结果增加非法值校验"""
    ...
    for cost_key, model in [
        ...
        excel_filename = f"ZD_{COST_TYPE_CN_MAP[cost_key]}_SHAP.xlsx"
...
# ===================== 工具函数：ZD数据字段校验 =====================
def _check_zd_data_fields(data_dict: dict):
    missing_fields = [f for f in Input_parameternames if f not in data_dict]
    if missing_fields:
        raise ValueError(f"❌ ZD数据缺失必填字段 → {missing_fields}")


# ===================== 核心函数1：ZD单条预测 =====================
def run_zd_prediction(data_dict: dict, model_map=None, ori_db_path=None, with_analysis=True):
    _check_zd_data_fields(data_dict)
    mi = ModelInput(data_dict)
    X = xgb.DMatrix([mi.to_list()], feature_names=Input_parameternames)
    loader = ZDModelLoader(model_map).load_all()
    predictor = ZDPredictor(loader, X)
    ...
    if with_analysis and ori_db_path:
        ...
        shap_result = generate_zd_shap_analysis(data_dict, loader)
...
# ===================== 核心函数2：ZD批量预测 =====================
def batch_run_zd_prediction(file_path, model_map, ori_db_path,type):
    file_path = Path(file_path).absolute()
    if not file_path.exists():
        raise FileNotFoundError(f"❌ ZD输入文件不存在 → {file_path}")
    file_suffix = file_path.suffix.lower()
    if file_suffix not in SUPPORT_FILE_TYPES:
        raise ValueError(f"❌ ZD仅支持{SUPPORT_FILE_TYPES}格式，当前为{file_suffix}")

    data_list = []
    if file_suffix == ".json":
        with open(file_path, 'r', encoding='utf-8') as f:
            single_data = json.load(f)
        _check_zd_data_fields(single_data)
        data_list.append(single_data)
    elif file_suffix == ".xlsx":
        print(f"📄 读取Excel文件（{type}），指定sheet：{type}")
        try:
            df = pd.read_excel(file_path, sheet_name=type)
        except Exception as e:
            raise ValueError(f"❌ 读取Excel的{type}sheet失败 → {str(e)}（请检查sheet名是否为「单筒/双筒」）")

        df.columns = [col.strip() for col in df.columns]
        missing_cols = [f for f in Input_parameternames if f not in df.columns]
        if missing_cols:
            raise ValueError(f"❌ ZD Excel缺失字段 → {missing_cols}")
        # 处理Excel中的非法值
    ...
    for row_idx, data_dict in enumerate(data_list, start=1):
        try:
            single_result = run_zd_prediction(
                data_dict=data_dict,
                model_map=model_map,
